...
def GetReward(reward, next_state):
  lidar_data = next_state[14:24]
  lidar_reward = (min(lidar_data.tolist()))/20
  velocity_reward =next_state[2]/10 # 최대 0.1
  return reward+lidar_reward+velocity_reward

def t(x):
    x = np.array(x) if not isinstance(x, np.ndarray) else x
    return torch.from_numpy(x).float().cuda() 

class Runner_otherReward2():
